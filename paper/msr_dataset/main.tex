...
asserts that HEAD, the porcelain status and the branch are unchanged after a
run. The other rebuilds every commit of a scripted history both ways and asserts
that the object-database read and a real checkout give the same nodes and the
same edges, over a macro-passed ref, a commented-out ref, a vendored tree and a
stem collision.
...
years and 989 drift events. The characterization script that drew Fig.~1 reports
3{,}577, 560.9 and 621, because it drops the nine snapshots at
...
of $N$. Snapshot cadence divides the same way, 39 days being the median
project's own median gap and 33 days the median pooled over every gap. Each
number is right for its stated scope, the release being the complete record and
...
models. Growth is heterogeneous rather than uniform, and the released
per-project trajectories carry the spread the medians hide.
...
topology, and the precedent that a workflow-structure corpus of roughly 180
units is publishable.
...
of my single audited project. And an event-archive frame would escape the
pagination cap that costs me 3{,}133 code-search hits.
...
\begin{table}[t]
\centering
\caption{Descriptor sensitivity to node insertion order, over 12 permutations
of each of 40 corpus graphs. Range is max minus min across permutations of the
same graph, so a reproducible descriptor sits at zero. D1 is the reason the
characterization excludes it.}
\label{tab:ordersens}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrc}
\toprule
Field & Median & Mean & Max & Released \\
\midrule
\texttt{D1\_csi} & 0.1429 & 0.1607 & 0.5714 & no \\
\texttt{D1\_n\_comm} & 0 & 0.9250 & 5 & no \\
\texttt{D2\_max\_gini} & 0 & 0 & 0 & yes \\
\texttt{D3\_alg\_conn} & 2.2e--15 & 4.6e--15 & 2.5e--14 & yes \\
\texttt{D3\_norm\_gap} & 1.9e--16 & 2.1e--16 & 8.4e--16 & yes \\
\texttt{D3\_fiedler\_bim} & 2.1e--07 & 0.0061 & 0.2424 & yes \\
\texttt{D4\_cycle\_rank\_norm} & 0 & 0 & 0 & yes \\
\bottomrule
\end{tabular}
\end{table}
...
order. Table~\ref{tab:ordersens} measures that, and \texttt{D1\_csi} moves by up
to 0.5714 against a published standard deviation of 0.123 while five of the
other descriptors do not move at all. Both D1 columns are excluded from every
figure. \texttt{D3\_fiedler\_bim} is not unstable but
...
peak of 2 nodes, because the models the extractor resolved were not the models
the project runs. Nothing detects that case.
