...
of the git object database without executing dbt and without a warehouse. The
corpus is a systematic sample of a documented frame of 3{,}718 candidates rather
than a census, and the screen that produced it ships with the data. Every edge
carries the parse mode that found it, so a stricter reader reconstructs a
stricter graph exactly. The strongest of three findings is that the staging,
intermediate and mart taxonomy does not describe the median public dbt project
at all.
\end{abstract}
...
reference backend and Egeria is a second one. All three are mature and adopted,
and the events they carry describe a company's own warehouse, so none can
publish a historical record. Repository reconstruction is the remaining route.
...
My contributions are the corpus of 3{,}586 snapshots with a graph and 15
descriptors at each, the unit-tested extraction tool, a characterization
establishing that the canonical dbt layer taxonomy fails to describe the median
public project, and the documented frame with its manifest of 626 checksummed
files.
...
of those hits. Three strata were unioned and deduplicated. S1 is a widely used
curated list of 20 entries, which a reader reaches for first and which yields
two projects. S2 is repository search over five dbt topics, each split into ten
star bands for the same reason. S3 is the code search above. The union is
3{,}718 candidates, released in full.
...
one. HEAD was recorded and never used to exclude, because a HEAD test is blind
to the adoption-and-abandonment lifecycle a longitudinal corpus should contain.
One project used dbt for two years, stopped, and still contributes 27
snapshots.
...
after it is partly circular on that sample and the release says so.
...
checkout-per-snapshot predecessor. Two tests hold the shortcut honest. One
asserts the worktree is untouched. The other rebuilds every commit of a scripted
history both ways and asserts the object-database read and a real checkout give
the same nodes and edges, over a macro-passed ref, a commented-out ref, a
vendored tree and a stem collision.
...
fidelity. \texttt{edges/<id>.csv.gz} holds every edge of every
snapshot, so a reader can rebuild any graph rather than trust the scalars.
\texttt{drift\_events.csv} records 989 consecutive-snapshot changes above
20\,\%, and \texttt{sampling\_frame.csv} with \texttt{excluded.csv} carry the
3{,}718 candidates and the 488 post-extraction rejections, each with a reason. \texttt{schema.json} is the machine-readable dictionary and
\texttt{MANIFEST.json} the provenance.

Two counts of this corpus circulate and I reconcile them here rather than let a
reader find the gap. The release carries all 3{,}586 snapshots, 566 cumulative
years and 989 drift events. The characterization script behind Fig.~1 and
Fig.~2 reports 3{,}577, 560.9 and 621, because it drops the nine snapshots at
$N \leq 1$ where density is undefined, excludes D1 as order-dependent, and gates
the giant-component descriptors to projects whose median component clears half
of $N$. Cadence divides the same way, 39 days per project against 33 pooled.
Each number is right for its scope, the release being the complete record and
the script the analyzable subset.
...
hides 43{,}893 of 503{,}925 edges, 8.7\,\%, which implies a uniform tax. The
distribution says otherwise. The median project loses nothing, 99 of the 154
lose no edge at all, the ninetieth percentile is 8.5\,\% and the worst loses
54.4\,\%. Fidelity is a per-repository
...
intermediate models and the median share of mart models are both exactly 0.00,
and at their final snapshot 56 projects have more unclassified models than the
three named layers put together. Layer labels come from naming
...
and the spread around the median is the part a single multiple hides.
...
withholds the SQL, so anyone sizing a dbt study from search counts should expect
to lose roughly a third of the hits.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{fig4_acyclicity_by_composition.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\columnwidth]{fig2_growth_normalized.pdf}
\caption{Node and edge counts normalized to each project's own first snapshot,
all 154 projects, corpus median and interquartile range in black. The log
ordinate spans three orders of magnitude.}
\label{fig:growth}
\end{figure}
...
record how the transformation topology above it changes, and their 195 is a
precedent for my 154 rather than a comparison I need to win.
...
repeat it. Two things differ. APEX-DAG infers dataflow where I read an explicit
declaration, and it works on single snapshots and releases no dataset, where the
longitudinal axis and the release are the whole of my contribution. The sentence
I claim is narrow. This is the first publicly available longitudinal corpus of
data-transformation lineage.

\textbf{Workflow execution traces.} WfCommons~\cite{coleman2022} publishes
traces of workflow runs, a record of execution rather than of repository-derived
topology, and the precedent that a corpus of roughly 180 units is publishable.
...
governance, and its release~\cite{hari2026} covers 106 snapshots of two estates. This corpus
supersedes it by 34 times in snapshots and 77 in projects, and it exists because
two estates cannot separate a real effect from a coincidence of two
organizations. No other use has been published.
...
than a birth and a death. A \texttt{dbt compile} manifest for the projects that
still build would give a ground-truth calibration set instead of my single
audited project. And an event-archive frame would escape the pagination cap that
costs me 3{,}133 code-search hits.
...
open-source databases, and 566 project-years with 989 marked drift events let
their named shapes be tested on a different artifact.

Does documentation keep pace with structure? Every snapshot carries
\texttt{doc\_rate} and \texttt{test\_rate} beside $N$, and the 119 projects with
a non-constant series give a median within-project Spearman $\rho$ of $-0.152$
against size, a weak negative worth a proper treatment.

Does layer composition converge as independently developed estates mature? The
institutional-convergence question becomes testable without interviewing
anybody.
...
order. Over 40 corpus snapshots at twelve node orderings each, \texttt{D1\_csi}
ranges by up to 0.5714 against a corpus standard deviation of 0.123, while D2
and D4 do not move at all and the two stable D3 fields agree to
$2.5\times10^{-14}$. Both D1 columns are excluded from every figure. \texttt{D3\_fiedler\_bim} is not unstable but
...
peak of 2 nodes, because the models the extractor resolved were not the ones the
project runs, and nothing detects that case.

\section{Availability}

Corpus, tool, frame and manifest are MIT licensed under concept DOI
\href{https://doi.org/10.5281/zenodo.20099999}{10.5281/zenodo.20099999}, with a
\texttt{requirements.txt} and a command line per stage. Data and code carry
separate version DOIs.
