...
3.9 for D1 and 15.0 for D4. Validation on an anonymized production
dbt lineage graph (223 nodes, 263 edges, 26 domains) yields
permutation-based Spearman correlations between D3 algebraic
connectivity and documentation rate ($\rho = -0.71$, permutation
$p = 0.002$, FDR-adjusted $p = 0.043$). This association survives
partial correlation controlling for domain size, edge count, and
source fraction (partial $\rho = -0.55$, $p = 0.021$). A
degree-preserving rewiring null model confirms that D1, D3 Fiedler
bimodality, and D4 H1 features capture structure beyond what degree
sequences alone determine ($|z| > 2.9$). These results, drawn from
a single case study, suggest that lineage topology encodes
governance-relevant structural signal, though generalization
requires validation across additional organizations.
\end{abstract}
...
generators and single dbt instance, we compute D1--D4 on two open
datasets: 11 scientific workflow DAGs from WfCommons
\citep{coleman2022wfcommons} spanning astronomy, bioinformatics, and
agroecosystem applications (101--4,846 nodes), and 2 data warehouse
schemas from DW-Bench \citep{dwbench2025} (OMOP clinical data model,
TPC-DI brokerage; 31--37 tables). These lack governance metadata, so the
comparison is purely structural: do real pipeline architectures from
different domains occupy distinguishable regions of descriptor space?
...
WfCommons \citep{coleman2022wfcommons} (101--4,846 nodes), two data
warehouse schemas from DW-Bench \citep{dwbench2025} (31--37 tables), and
the production dbt manifest.

\begin{table}[htb]
\centering
\caption{Structural profiles across 14 graphs from three sources.
  WfCommons values are means with ranges in parentheses.}
\label{tab:crossdataset}
\begin{tabular}{lrrrrrr}
\toprule
Source & $n$ & CSI & max Gini & gap & Fiedler bim. & CR/N \\
\midrule
WfCommons (11) & 101--4846 & 0.78 & 0.39 & 0.011 & 0.56 & 0.91 \\
 & & \footnotesize{(.53--.84)} & \footnotesize{(.00--.80)} & \footnotesize{(.00--.04)} & \footnotesize{(.08--.93)} & \footnotesize{(.00--2.99)} \\
...
The production dbt lineage has the highest CSI (0.95) of any graph
tested, and the lowest Fiedler bimodality (0.30) and cycle rank
per node (0.35). Scientific workflows show wide topological variation:
seismology pipelines are nearly tree-structured (CR/N = 0), while
bioinformatics alignment workflows (BWA, BLAST) have CR/N $> 1.9$.
DW-Bench schemas occupy an intermediate position.

These results confirm that the descriptors produce interpretable
variation across genuinely different pipeline architectures, not
only synthetic governance variants. The dbt manifest's high CSI and
low cycle rank relative to unstructured scientific workflows is
consistent with governed production pipelines exhibiting different
structural signatures from ad hoc computational workflows, though
alternative explanations (domain differences, scale effects) cannot
be ruled out from this comparison alone.
...
D3 algebraic connectivity produces the strongest real-data signal
($\rho = -0.71$ vs.\ documentation, FDR $p = 0.043$) and survives
partial correlation. However, the null model shows it is
indistinguishable from degree-preserving rewirings ($z = -0.42$),
meaning its value is largely determined by degree sequence. This is
not necessarily disqualifying: degree sequence itself may encode
governance-relevant information (well-governed domains may have
characteristic degree distributions), but it means algebraic
connectivity is not capturing ``hidden'' structural signal beyond
what simple degree statistics provide.
...
documentation rate ($\rho = -0.71$) warrants explanation. Source-only
domains have moderate documentation (41\%) but zero internal edges,
yielding algebraic connectivity of zero. Transformation-heavy domains
(silver/intermediate) have high internal connectivity but zero
documentation. The well-governed gold/mart domain sits in between.
This pattern reflects a genuine organizational dynamic: documentation
effort is concentrated at data ingestion points and final outputs,
not at intermediate transformations.

\subsection{Limitations}

\textbf{Single case study.} The real-data validation uses one
organization's dbt lineage. The results support feasibility, not
generalization. Cross-organizational validation requires additional
datasets. Three open corpora are candidates: DLG-DG-23
\citep{dlgdg2024} provides 18 real-world data lineage graphs from
Huawei Cloud (278--17,085 nodes) with governance labels;
WfCommons \citep{coleman2022wfcommons} offers 180 scientific workflow
DAGs whose task-dependency structure is topologically analogous to
data lineage; and DW-Bench \citep{dwbench2025} provides five data
warehouse schemas with FK and lineage edges.
...
All code, anonymized data, and experiment scripts are available at
