...
relationship. Layer-stratified permutation gives $p = 1.0$, confirming
the correlation is captured entirely by between-layer architecture. A
...
We frame the paper as feasibility evidence and methodological scaffolding
rather than validated governance prediction. Sufficient validation of
the governance hypothesis requires curated domain-level metadata from
multiple organisations under partnership arrangements; we outline this
...
\begin{enumerate}
  ...
  \item \textbf{Negative cross-organisation pilot with methodological
        caveat.} D3/documentation does not replicate to two public dbt
        projects (Cal-ITP, Mattermost) under folder-derived domain
        partitioning. We discuss why folder-derived domains are not
        the same construct as metadata-assigned dbt domains, and we
        outline what curated cross-organisation validation would require.
\end{enumerate}
...
not be replicable from public dbt projects alone.
...
As a single-organisation governance pilot, we found a 3-tier rank
ordering between D3 algebraic connectivity and documentation rate in
one dbt lineage ($\rho = -0.71$, bootstrap CI $[-0.92, -0.27]$). Layer-
stratified permutation ($p = 1.0$) showed the relationship is captured
entirely by between-layer architecture, not within-layer governance
variation. The effect size sits at the detection threshold for the
observed sample structure. A preliminary cross-organisation comparison
on two public dbt projects did not replicate the dbt finding, but
construct mismatch between metadata-assigned and folder-derived domains
prevents a clean conclusion about replicability.

The paper's contribution is therefore: a validated structural
characterization framework, methodological scaffolding for governance--
topology studies (permutation inference, bootstrap CIs, null models,
subset robustness, layer-stratified permutation, seed robustness, power
analysis), a single-organisation pilot establishing the feasibility of
the analysis pipeline, and an explicit boundary condition: cross-
organisation governance validation requires curated domain-level
metadata that is not available from public dbt projects. We outline
what such validation would require and pursue it in future work.
