...
descriptor space distinct from scientific workflows. Cross-organization validation on two additional public dbt projects
(Cal-ITP, 26 domains; Mattermost, 6 domains) finds no replication:
the D3 correlation does not reach significance in Cal-ITP and reverses
sign in Mattermost, consistent with the between-layer architecture
interpretation. The descriptors produce reproducible structural
characterization across organizations; governance prediction requires
within-organization calibration and cannot be generalized from a
single production case; cross-organization validation shows no replication.
\end{abstract}
...
\begin{enumerate}
  ...
  \item Initial real-data validation on an anonymized production dbt
        ...
        inference, partial correlations, subset robustness analysis, and
        layer-stratified permutation showing D3 captures between-layer
        architecture, not within-layer governance quality.

  \item Cross-dataset structural characterization of 32 graphs from
        workflow, lineage, warehouse-schema, and dbt sources, showing
        that production lineage graphs occupy a distinct descriptor
        region from scientific workflows.

  \item Cross-organization validation on two public dbt projects (Cal-ITP,
        Mattermost) showing the D3/documentation association does not
        replicate and reverses sign, establishing a principled boundary
        condition for topology-based governance inference.
\end{enumerate}
...
other descriptors survive FDR correction. (Table~\ref{tab:subset}
reports FDR $p = 0.029$ for the same Subset~A result; the difference
arises because the two tables apply BH correction to different
hypothesis families: Table~\ref{tab:exp2} corrects across all
descriptor--target pairs in Experiment~2, while Table~\ref{tab:subset}
corrects within the subset-robustness family only.)
Experiment~2e subsequently shows this association is a between-layer
architectural pattern rather than a within-layer governance signal.
...
Neither Cal-ITP nor Mattermost replicates the dbt D3 correlation.

\begin{table}[htb]
\centering
\caption{D3 algebraic connectivity vs.\ documentation rate across three
  organizations. The sign of the correlation reverses between
  organizations, confirming that D3 captures organization-specific
  layer architecture rather than a universal governance signal.}
\label{tab:crossorg}
\begin{tabular}{lrrrr}
\toprule
Organization & Domains & $\rho$ & perm.\ $p$ & Direction \\
\midrule
dbt (this study) & 18 & $-0.708$ & 0.002 & negative \\
Cal-ITP & 26 & $+0.023$ & 0.911 & none \\
Mattermost & 6 & $+0.812$ & 0.070 & positive \\
\bottomrule
\end{tabular}
\end{table}
...
(all $p > 0.11$). Mattermost (6 domains) shows a positive marginal
association for D3 ($\rho = +0.81$, $p = 0.070$), the opposite sign
from our dbt case study. The sign reversal arises from organizational
architecture: in Mattermost, the best-documented domains are the
large, internally-connected mart domains, whereas in our dbt case the
best-documented domains are source-only clusters with zero internal
edges. The direction of any D3--documentation relationship depends
entirely on how governance practices align with architectural layer
choices in a specific organization.

No descriptor achieves FDR-corrected significance in either new
organization. This is the expected result given the between-layer
interpretation established in Experiment~2e: the observed dbt
correlation was layer composition, not within-layer governance
quality, and layer composition varies by organization.
...
\textbf{Governance correlation does not generalize.} The original D3/documentation
correlation comes from one anonymized production dbt case. Two public dbt
replications (Experiment~6) did not reproduce the association: Cal-ITP
($n = 26$ domains) shows no correlation, and Mattermost ($n = 6$) shows a
positive trend in the opposite direction. We interpret D3/documentation
as an organization-specific layer-architecture effect. The structural
characterization results (D1 null model, cross-dataset clustering) are
robust across datasets; the governance-prediction interpretation requires
local calibration and cannot be generalized without further
governance-labeled lineage data from additional organizations.
...
D1 differentiation at large scale and the degree-preserving null model
results are the most robust findings. D3 normalized spectral gap was
confounded by domain size; D3 algebraic connectivity captures
organization-specific layer architecture rather than universal governance
quality, as confirmed by cross-organization non-replication. D4 persistent
features add value over cycle rank in null model testing but not in direct
governance correlations. These negative findings are as informative as
the positive ones: they establish where topology-based governance
inference is and is not warranted.
